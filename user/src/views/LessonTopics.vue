      ...
      <div v-for="(topic, idx) in topics" :key="topic.id" class="topic-card geo-card">
        ...
        <div v-if="expanded.has(topic.id)" class="topic-expanded">
          <div class="content-block">
            ...
            <RouterLink :to="`/topics/${topic.id}/read`" class="read-more-btn">
              <FileText :size="14" /> Batafsil o'qish (2-sahifa) →
            </RouterLink>
          </div>

          <div class="action-grid">
            <RouterLink :to="`/topics/${topic.id}/test?mode=open`" class="action-card action-open">
            ...
            <RouterLink :to="`/topics/${topic.id}/test?mode=closed`" class="action-card action-closed">
