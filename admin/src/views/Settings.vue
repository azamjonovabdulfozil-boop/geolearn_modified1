<template>
  <div class="fade-in settings-wrap">
    ...
    <div class="settings-grid">
      ...
      <div class="geo-card section-card span-2">
        ...
        <div class="avatar-row">
          ...
          <div class="avatar-info">
            ...
            <p class="avatar-role">{{ settings.t('teacher_role') }}</p>
      ...
      <!-- Brand (CRM name / logo) — teacher only -->
      <div class="geo-card section-card">
        <div class="section-head">
          <div class="section-icon" style="background:hsl(174 65% 35%/0.12)">
            <Sparkles :size="16" style="color:hsl(174 60% 40%)" />
          </div>
          <h2>{{ settings.t('brand') }}</h2>
        </div>
        <div class="brand-row">
          <div class="brand-logo-box">
            <img v-if="brandLogoPreview || settings.brandLogo" :src="brandLogoPreview || settings.brandLogo" class="brand-logo-img" />
            <Globe v-else :size="28" style="color:hsl(var(--muted-fg))" />
          </div>
          <div class="brand-info">
            <div class="field-row">
              <label>{{ settings.t('brand_name') }}</label>
              <input v-model="brandForm.name" class="geo-input" placeholder="GeoLearn" />
            </div>
            <div class="brand-actions">
              <label class="geo-btn-outline btn-sm cursor-pointer">
                <ImageIcon :size="13" /> {{ settings.t('brand_logo') }}
                <input type="file" accept="image/*" class="hidden" @change="onBrandLogoChange" />
              </label>
              <button @click="resetBrand" class="geo-btn-ghost btn-sm">
                <RefreshCw :size="13" /> {{ settings.t('brand_reset') }}
              </button>
            </div>
          </div>
        </div>
        <p v-if="brandMsg" class="status-msg ok">{{ brandMsg }}</p>
        <button @click="saveBrand" class="geo-btn-primary w-full">
          <Save :size="15" /> {{ settings.t('save_profile') }}
        </button>
      </div>
      ...
      <div class="geo-card section-card">
        ...
        <div class="option-grid">
          <button v-for="[val, key, Icon] in themeOptions"
            :key="val" @click="settings.setTheme(val)"
      ...
      <div class="geo-card section-card">
        ...
        <div class="font-grid">
          <button v-for="[val, key, sz] in fontOptions"
            :key="val" @click="settings.setFontSize(val)"
...
<script setup>
import { ref } from "vue";
import { Sun, Moon, Lock, Loader2, Save, Palette, Languages, UserCircle, Type, ImageIcon, Trash2, Sparkles, Globe, RefreshCw } from "lucide-vue-next";
...
async function saveProfile() {
  ...
  try {
    ...
    profileMsg.value = settings.t('saved_ok');
...
.avatar-box {
  ...
  background: linear-gradient(135deg, hsl(var(--primary)), hsl(190 80% 42%)); color: white;
  font-size: 20px; font-weight: 800;
  display: flex; align-items: center; justify-content: center;
  flex-shrink: 0; overflow: hidden; box-shadow: 0 8px 18px hsl(var(--primary)/.22);
}
...
/* Brand */
.brand-row { display: flex; gap: 12px; align-items: center; flex-wrap: wrap; }
.brand-logo-box {
...
.brand-info { flex: 1; min-width: 180px; display: flex; flex-direction: column; gap: 8px; }
.brand-actions { display: flex; gap: 6px; flex-wrap: wrap; }
...
.option-btn {
  ...
  cursor: pointer; transition: all .15s; color: hsl(var(--muted-fg));
}
